...
- `SECRET_KEY` — секретный ключ проекта(необязательно)

## Заполнение базы покемонов

Проще всего это сделать при помощи админки. Как это делается:
1. В командной строке создайте суперпользователя:
```commandline
python manage.py createsuperuser
```
2. Следуйте инструкциям, введите логин, email, пароль.
3. Запустите разработческий сервер

```sh
python3 manage.py runserver
```
4. Перейдите по ссылке [http://127.0.0.1:8000/admin/](http://127.0.0.1:8000/admin/)
5. Осталось добавить покемонов и их характеристики.
